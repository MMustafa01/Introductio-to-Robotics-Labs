...
%     figure; imshow(red_mask); title("Post Morphological Operations"); figure;
    ...
    if size(red_cubes) ~= 0
        for i = 1:size(red_cubes)
            if (~(800 <= red_cubes(i,1) && red_cubes(i,1) <= 1137 && 282 <= red_cubes(i,2) && red_cubes(i,2) <= 611)) && (~(1150 <= red_cubes(i,1) && red_cubes(i,1) <= 1400 && 164 <= red_cubes(i,2) && red_cubes(i,2) <= 420)) && 332<= red_cubes(i,1) && red_cubes(i,1) <= 1608
...
%     end
    % alignment check
    plot(ones(1080)*960,1:1:1080)
    plot(1:1:1920,ones(1920)*540)
